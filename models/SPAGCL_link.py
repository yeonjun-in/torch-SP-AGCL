...
from utils import get_graph_drop_transform, set_everything, dense_to_sparse_x, to_dense_subadj, get_data
from copy import deepcopy
from collections import defaultdict
from torch_geometric.utils import to_undirected, to_dense_adj, dense_to_sparse, subgraph, add_self_loops, coalesce
...
class SPAGCL_link(embedder):
    ...
    def training(self):
        ...
        for seed in range(self.args.seed_n):
            ...
            link_data = torch.load(f'dataset/link/{self.args.dataset}_link.pt')
            train_pos, train_neg, train_label = link_data['train_edges'].T, link_data['train_edges_neg'].T, link_data['train_label'].T
            test_pos, test_neg, test_label = link_data['test_edges'].T, link_data['test_edges_neg'].T, link_data['test_label'].T

            edge_index = train_pos.clone()
            train_edge = torch.cat((train_pos, train_neg), dim=1)
            test_edge = torch.cat((test_pos, test_neg), dim=1)
                
            if self.args.ptb_rate > 0.0:
                clean_data = get_data(self.data_home, self.args.dataset, self.args.attack, 0.0)[0]
                clean_adj = to_dense_adj(clean_data.edge_index)[0]
                ptb_adj = to_dense_adj(self.data.edge_index)[0]
                ptb_edge = (ptb_adj > clean_adj).nonzero().T

                edge_index = coalesce(torch.cat((edge_index, ptb_edge), dim=1))
                train_edge = torch.cat((train_edge, ptb_edge), dim=1)
                train_label = torch.cat((train_label, torch.ones(ptb_edge.size(1))))
        
            data.edge_index = edge_index
            data.train_edge_index, data.val_edge_index, data.test_edge_index = train_edge, test_edge, test_edge
            data.train_label, data.val_label, data.test_label = train_label, test_label, test_label
            ...
            for epoch in range(1, self.args.epochs+1):
                ...
                if epoch > self.args.warmup:
                    ...
                    loss += self.model.loss(z1, z_knn, batch_size=0)
                    loss += self.model.loss(z1, z_adv, batch_size=0)
                ...
                if epoch % self.args.verbose == 0:
                    _, val_acc, _ = self.verbose_link(data)
                    if val_acc > best:
                        best = val_acc
                        cnt_wait = 0
                        best_epochs = epoch
                        torch.save(self.model.online_encoder.state_dict(), '{}/saved_model/best_{}_{}_{}_{}_seed{}.pkl'.format(self.args.save_dir, self.args.dataset, self.args.attack, self.args.attack_type, self.args.embedder, seed))
                    else:
            ...
            self.best_epochs.append(best_epochs)
            self.model.online_encoder.load_state_dict(torch.load('{}/saved_model/best_{}_{}_{}_{}_seed{}.pkl'.format(self.args.save_dir, self.args.dataset, self.args.attack, self.args.attack_type, self.args.embedder, seed), map_location=f'cuda:{self.args.device}'))
            only_clean = True if self.args.dataset in ['photo', 'computers', 'cs', 'physics', 'amz', 'amz2', 'squirrel', 'chameleon',] else False
            self.eval_link(data)
